...
class ArticleXinminSpider(Spider):
  name = "article_xinmin"
  allowed_domains = ["tech.xinmin.cn"]
  start_urls = [
    'http://tech.xinmin.cn/lb.html'
  ]
  ...
  def parse_list(self, response):
    ...
    news_url_list = xpath.list('//*[@class="MP_listpagelist"]/ul/li/div/h2/a/@href')
    time_list = xpath.list('//*[@class="MP_listpagelist"]/ul/li/div/p[@class="info"]/span[@class="info2"]/text()')
    ...
    if continue_next:
      next_url = xpath.first('//*[@title="next page"]/@href')
      if next_url:
        yield Request('http://tech.xinmin.cn/' + next_url, callback=self.parse_list)

  def parse_news(self, response):
    xpath = XPath(Selector(response))
    i = WebArticleItem()

    i['title'] = xpath.first('//h1[@id="MP_title"]/text()')
    i['url'] = response.url

    source = '\n'.join(xpath.list('//*[@id="MP_info"]//text()'))
    ...
    i['content'] = xpath.first('//*[@id="MP_article"]')
    i['siteName'] = 'tech.xinmin.cn'
