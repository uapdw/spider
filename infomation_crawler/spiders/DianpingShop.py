...
from infomation_crawler.items import DianPingShopItem
import datetime
import pymongo
...
class DianpingShopSpider(CrawlSpider):
	name = 'DianpingShop'
	allowed_domains = ['dianping.com']
	urls = []
	temp = "0_%E5%A4%A7%E5%A8%98%E6%B0%B4%E9%A5%BA"
	for i in range(1,2507):
		urls.append(
	...
	rules = [
			...
			Rule(SgmlLinkExtractor(allow=(r'http://www.dianping.com/shop/\d+'),deny=r'(review|14712376|14128993|17834214)'),callback='parse_item'),
			]
	...
	def parse_item(self, response):
		...
		area = sel.xpath('//div[@class="expand-info address"]/a/span/text()').extract()
		detail = sel.xpath('//div[@class="expand-info address"]/span[2]/text()').extract()
		address = (len(area)>0 and area[0].strip() or '') + " " + (len(detail)>0 and detail[0].strip() or '')
