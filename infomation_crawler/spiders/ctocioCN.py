...
class CtocioCNSpider(CrawlSpider):
  ...
  start_urls = ['http://www.ctocio.com.cn/']
  ...
  rules = (
    Rule(SgmlLinkExtractor(allow=r'(server|www|server|news|bigdata|mobile|storage|database|esoft|networking|security|os|cio|virtualization)\.ctocio\.com\.cn/(.*)\d+/\d+\.shtml'), callback='parse_item'),
  )
